...
//       await client.execute(`select status, role from usuarios where status != 'deleted'`)
...
//       await client.execute(`select priority from admin_notifications where status = 'unread'`)
//     ]);
...
//     let query = await client.execute(`select * from usuarios where status != 'deleted' order by created_at desc`)
//       .then(res => res.rows)
//       .then(rows => rows.map(row => ({
//         ...row,
//         access_logs: {
//           count: row.access_logs_count
//         }
//       })))

//     // Filtros opcionales
//     if (status) {
//       query = query.filter(u => u.status === status);
//     }
    
//     if (role) {
//       query = query.filter(u => u.role === role);
//     }
