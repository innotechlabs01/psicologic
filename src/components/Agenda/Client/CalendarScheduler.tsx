...
export const CalendarScheduler = () => {
    ...
    const renderDays = () => {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            if (isToday) className += ' border-indigo-500 bg-indigo-50';
            if (isPast) className += ' opacity-60 cursor-not-allowed bg-gray-50';
            else className += ' hover:bg-indigo-50 cursor-pointer';
    ...
    return (
        <div className="flex flex-col lg:flex-row gap-8 w-full max-w-6xl mx-auto bg-white p-6 rounded-xl shadow-xl">
            <div className="flex-1">
                ...
                <div className="grid grid-cols-7 gap-3 text-center mb-2 font-semibold text-gray-500 text-sm">
                ...
                <div className="grid grid-cols-7 gap-3">
                    {loading ? (
                        ...
                        Array.from({ length: 42 }).map((_, i) => (
                            <div key={`sk-${i}`} className="h-20 rounded-lg bg-gray-100 dark:bg-gray-800 animate-pulse" />
                        ))
                    ) : (
                        renderDays()
                    )}
