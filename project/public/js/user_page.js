...
// Definición de las cartas disponibles
const QUESTIONS = [
...
function createCardElement(cartaId, gameType, pregunta) {
...
function setupSidebar() {
  ...
  // Agregar estados visuales iniciales al menú
...
async function loadUser() {
  try {
    ...
    // Si no tenemos datos del usuario pero tenemos token, intentar obtener del servidor
    if (!userData && token) {
      try {
    ...
    // Si aún no tenemos datos pero tenemos email, crear objeto básico
...
async function loadSidebarMenu(items) {
  try {
    // Primero cargar el navbar si no está cargado
    ...
    items.forEach(item => {
      if (item.active && user.accessibleSections.includes(item.section)) {
        ...
        a.addEventListener("click", async (e) => {
          ...
          // Primero intentamos crear la sección si no existe
          ...
          if (!existingSection) {
            ...
            if (mainContent) {
              ...
              // Si es la sección de pagos, crear la estructura completa
...
async function loadPaymentSection() {
  try {
    // Asegurarnos de que el contenedor de la sección de pagos existe
    const paymentSection = document.getElementById('payment');
    if (!paymentSection) {
      throw new Error("Sección de pagos no encontrada");
    }

    // Cargar el contenido de paymentMethod.html
...
async function loadUserPayments(user = null) {
  try {
    ...
    // Verificar si hay un paymentId en la URL
    ...
    if (paymentId) {
      // Si hay un paymentId en la URL, intentar obtener ese pago específico
    ...
    // Si no hay paymentId o el pago específico no se encontró, obtener estado general
    ...
    // Si la respuesta es exitosa o es 404, procesamos normalmente
...
function updateSubscriptionStatus(data) {
  ...
  // Usar paymentDate en lugar de lastPayment si está disponible
  ...
  // Usar nextPaymentDate en lugar de expirationDate si está disponible
  ...
  // Mostrar botón de pago si es necesario
...
function showDefaultSection() {
  ...
  // Remover estado activo de todas las secciones primero
...
function setupAdminLogic() {
  ...
  if (generarBtn) {
    generarBtn.addEventListener('click', async () => {
      ...
      const gameType = juegos.value; // 'besos', 'fuego', o 'rayo'
      
      // Limpiar secciones de jugadores existentes
      const listaSelecciones = document.getElementById('listaSelecciones');
      if (listaSelecciones) {
        listaSelecciones.innerHTML = '';
      }

      // Inicializar las secciones de jugadores con el tipo de juego seleccionado
...
function getCurrentUser() {
  try {
    ...
    // Si no hay usuario en localStorage, intentar obtener del email
    const userEmail = localStorage.getItem('userEmail');
    if (userEmail) {
      return {
        id: userEmail, // Usar email como ID temporal
        email: userEmail,
        name: userEmail.split('@')[0],
        accessibleSections: ["cardGame", "game2", "game3", "payment", "assistance"]
      };
    ...
    // Si todo lo demás falla, devolver un objeto con secciones accesibles pero sin identificación
...
function initializePlayerSection(playerName, gameType = 'besos') {
  ...
  // Crear el contenido de la sección del jugador
  playerDiv.innerHTML = `
...
function generateCardValues(totalCards) {
  ...
  const availableValues = QUESTIONS.map(c => c.value).slice(0, maxPairs);

  for (let i = 0; i < maxPairs; i++) {
    values.push(availableValues[i % availableValues.length]);
    values.push(availableValues[i % availableValues.length]);
  }
...
function setupSocketListeners() {
  // Almacenar el estado de cada jugador
  ...
  function handleCardUpdate(user, cartaId, pregunta, flipped, gameType, timestamp) {
    ...
    // Obtener o inicializar el estado del jugador
    ...
    if (!card) {
      ...
      card = createCardElement(cartaId, currentGameType, pregunta);
    }

    // Si la carta está volteada (seleccionada), será la nueva primera carta
    if (flipped) {
      // Mover la carta actual primera (si existe) al contenedor de otras cartas
      ...
      // Establecer esta carta como la primera
      ...
      // Si la carta se está deseleccionando
      playerCards.querySelector('.other-cards-container').appendChild(card);
    }

    // Aplicar animaciones y efectos visuales
  ...
  // Escuchar eventos de actualización de cartas
  ...
  // Escuchar eventos de selección de cartas
...
async function handlePayment() {
  try {
    ...
    const response = await fetch('/api/payments/checkout', {
      ...
      body: JSON.stringify({
        userId: user.id,
        amount: 100000 // Monto fijo para la suscripción
      })
    });
...
function setupCards(gameType) {
    ...
    cardValues.forEach((question, index) => {
        ...
        div.innerHTML = `<div class="card ${gameType}"><div class="back"></div><div class="face"></div></div>`;
        
        const card = div.querySelector('.card');
        card.dataset.cartaId = question;
        
        const face = div.querySelector('.face');
        face.innerText = question;
        
        // Aplicar estilos según el tipo de juego
        if (gameType === 'fuego') {
