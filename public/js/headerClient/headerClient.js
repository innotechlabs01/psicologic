...
const SidebarManager = {
  ...
  renderMenu(menuItems, isDynamic) {
    ...
    return menuItems
      ...
      .map(item => {
        const label = typeof item.name === 'string' ? item.name : item.toString();
        const nameText = typeof item.nametext === 'string' ? item.nametext : item.toString();
        if (isDynamic && Array.isArray(item.subItem) && item.subItem.length > 0) {
          return this.renderDynamicSubMenu(nameText, item.subItem, item.id || label);
        }
  ...
  renderDynamicSubMenu(label, subItems, parentId) {
    ...
    const menuUniqueId = Math.random().toString(36).substring(2, 10);

    return `
      <button type="button" class="flex items-center w-full p-2 text-base text-gray-900 transition duration-75 rounded-lg group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700" data-collapse-toggle="${menuUniqueId}">
      ...
      <ul id="${menuUniqueId}" class="hidden py-2 space-y-2">
        ${subItems
        ...
        .map(nestedItem => {
          ...
          const nameTextNested = typeof nestedItem.nametext === 'string' ? nestedItem.nametext : nestedItem.toString();
          const pathEndPoint = label.toLowerCase() === 'juegos' ? `/client/games/${nestedLabel}` : `/client/settings/${nestedLabel}`;
          ...
          return `
            <li>
              <a href=${pathEndPoint} data-navigate class="flex items-center w-full p-2 text-base text-gray-900 transition duration-75 rounded-lg group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700 sidebar-link">
                ...
                <span class="flex-1 ms-3 text-left rtl:text-right whitespace-nowrap">${nameTextNested}</span>
              </a>
            </li>
  `;
        })
